...
__Apostrophes/single quotes need to be preceded by a backslash (e.g. `\'`). Dollar signs ($) must be replaced with `&#36;`. Commas not used to separate array elements need to be replaced with the ASCII code `&#044;`. Positioning using `px` can pose an issue for responsive design.__
...
__css (position 7):__ This field applies in-line CSS code to the `style` attribute of `div.webModInner`. __Positioning of the ICC needs to be relative to the lower left-hand corner, and within the pixel dimensions of the mobile image's usable space in order to render correctly for both desktop and mobile versions.__
...
### 2018-two-third-mod
This module spans two-thirds of the width of the view port at all viewport sizes. To do this it requires two image files. The smaller, or mobile, image (670px wide) renders for viewports under 1012px wide.  The larger (1230px wide) image renders for viewports 1012px and over. There is no option for a card-based pattern.
...
### 2018-three-quarter
This module spans three-quarters of the width of the view port for tablet landscape (over 1012px) viewports and larger. It spans half of the width of view ports table portriat view ports and below.

. To do this it requires two image files. The smaller, or mobile, image (670px wide) renders for viewports under 1012px wide.  The larger (1230px wide) image renders for viewports 1012px and over. There is no option for a card-based pattern.
